...
def breadthFirstSearch(problem):
    ...
    (state,toDirection,toCost) = frontier1.pop()
    reached1.append(state)

    while not problem.isGoalState(state):
        successors = problem.getSuccessors(state)
        for son in successors:
            if not son[0] in reached1:
                frontier1.push((son[0],toDirection + [son[1]],toCost + son[2]))
                reached1.append(son[0])
        (state,toDirection,toCost) = frontier1.pop()
...
def uniformCostSearch(problem):
    ...
    frontier2.push((problem.getStartState(),[],0), 0)
    (state,toDirection,totalCost) = frontier2.pop()
    reached2.append((state,totalCost))

    while not problem.isGoalState(state):
        succ = problem.getSuccessors(state)
        for child in succ:
            reachedE = False
            total_C = totalCost + child[2]
            for (visitedState,visitedToCost) in reached2:
                if(child[0] == visitedState) and (total_C >= visitedToCost):
                    reachedE = True
                    break

            if not reachedE:
                frontier2.push((child[0], toDirection + [child[1]], totalCost + child[2]), totalCost + child[2])
                reached2.append((child[0], totalCost + child[2]))
