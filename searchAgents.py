        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not isWall:
                ...
                for corner in self.corners:
                    ...
                    cornerIndex += 1
                if cornerIndex < 4:
                    cornerVisited_new[cornerIndex] = True
...
def foodHeuristic(state, problem):
    ...
    # the search method we use in the problem
    #So we could use this function to get the minimum distance from the current position to any
    # of the food point
    "*** YOUR CODE HERE ***"
    K = 0
    md = 0 #Max Distance
    for y in range(foodGrid.height):
        for x in range(foodGrid.width):
            if (foodGrid[x][y] == 1) and (mazeDistance(position,(x,y),problem.startingGameState) > md):
                md = mazeDistance(position,(x,y),problem.startingGameState)
    K = md
    return K
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        #return search.depthFirstSearch(problem)S
